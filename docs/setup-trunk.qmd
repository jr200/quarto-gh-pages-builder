...
quarto-graft trunk init "My Documentation" --template default
```
...
2. **Build process**: `quarto-graft trunk build` renders each graft in isolation
...
This shows each graft's build status (`current`, `stale`, `broken`, `never built`), last build time, and whether HEAD matches the last built commit.
