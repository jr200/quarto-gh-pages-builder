...
- `--overwrite/--no-overwrite` - Overwrite existing docs/ directory if it exists
...
quarto-graft trunk init "My Site" --template minimal

# Include addons
quarto-graft trunk init "My Docs" --with navbar-search --with dark-mode
```
...
2. Renders each graft in isolation
